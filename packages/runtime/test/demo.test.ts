...
describe('beat 1, the hook: the tree as it ships, and what the rule reaches', () => {
  it('checks ok at 205 documents, and describe computes the five routes the rule reaches', () => {
    expect(refusalsAt(dir)).toEqual([]);
    expect(documents()).toBe(205);
...
describe('beat 3, following the hints', () => {
  ...
  it('the finished route yields ok, at 206 documents', () => {
    paste('archive-customer.step3.trigger.json');
    expect(refusalsAt(dir)).toEqual([]);
    expect(documents()).toBe(206);
  });
});
