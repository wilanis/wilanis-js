...
const binding = 'features/monitor/data/monitor-store.binding.json';
...
describe('the scope the lowering fills, which no document writes', () => {
  it('carries the store read to a site over a scoped collection, and adds no node for it', () => {
    ...
    const nodes = nodesOf('@features/monitor/data/kept-get.graph.json');
    ...
    const plain = plainNodesOf('@features/monitor/data/kept-get.graph.json');
  ...
  it('carries none to a site over a collection the store does not scope', () => {
    ...
    const nodes = nodesOf('@features/monitor/data/store-and-latest.graph.json');
  ...
  it('carries none where nothing is scoped, so a tree that scopes nothing lowers as it did', () => {
    ...
    expect(plainNodesOf('@features/monitor/data/kept-get.graph.json').asked.in.scope).toBeUndefined();
  });
  it('carries the read a binding delegation names, since a delegation is a site like any other', () => {
    ...
    const spec = scopedTree(
      load => compilerOf(load, 'local').operation('@monitor/domain/monitor.port.json#listAll').spec,
      {
        [binding]: (doc: any) => {
