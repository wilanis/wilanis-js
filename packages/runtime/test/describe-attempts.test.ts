 ...
 *
 * The example's own `@http/http.port.json` and `@blob/csv.port.json` carry the promises, and the example itself
 * declares the retries: on get-row's request, and on the REST binding's `listAll`.
 */
...
describe('describe: what a call site declares about its tries', () => {
  it('appends the retry and the bound to the line of the node that declares them', () => {
    expect(describeDoc(example, '@customers/data/get-row.graph.json')).toContain(
  ...
  it('leaves the line of a node that declares neither as it was', () => {
    expect(describeDoc(example, '@customers/data/list-rows.graph.json')).toMatch(
      /^ {4}fetched {2}@http\/http\.port\.json#request$/m,
    );
  });

  it('says a retry with nothing but its count as the count alone', () => {
    expect(describeDoc(example, '@customers/data/customers-rest.binding.json')).toContain(
  ...
  it('leaves a binding operation that declares neither as it was', () => {
    expect(describeDoc(example, '@customers/data/customers-rest.binding.json')).toMatch(
