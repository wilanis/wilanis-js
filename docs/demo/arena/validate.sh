...
# register one silver and one bronze customer as bo, then the nine calls in the same order with the same bodies.
#
...
say "## the new route in map"; npx wilanis map . --profile local 2>&1 | grep -A6 -i "toggl\|active" | head -24 | tee -a "$OUT"

free_port
CUSTOMERS_JWT_SECRET=arena-secret CUSTOMERS_DATABASE_URL=postgres://unused npx wilanis start . --profile local > "$LOG" 2>&1 &
PID=$!
...
# reloads, one per sign-in, before registering, or one empties the memory store under the curls below.
...
ADAID=$(curl -s -g -X POST "$H/customers" -H "authorization: Bearer $BO" -H 'content-type: application/json' -d '{"name":"Ada Lovelace","email":"ada@example.com","tier":"silver"}' | sed -n 's/.*"id":"\([^"]*\)".*/\1/p')
GRACEID=$(curl -s -g -X POST "$H/customers" -H "authorization: Bearer $BO" -H 'content-type: application/json' -d '{"name":"Grace Hopper","email":"grace@example.com","tier":"bronze"}' | sed -n 's/.*"id":"\([^"]*\)".*/\1/p')
say "registered silver customer $ADAID and bronze customer $GRACEID as bo"
c "1 toggle a silver customer as bo (expect active true)"           -X "$METHOD" "$H$(R "$ADAID")" -H "authorization: Bearer $BO"
c "2 toggle them again as bo (expect active false)"                 -X "$METHOD" "$H$(R "$ADAID")" -H "authorization: Bearer $BO"
c "3 read it back"                                                   "$H/customers/$ADAID"
c "4 toggle a bronze customer as bo"                                 -X "$METHOD" "$H$(R "$GRACEID")" -H "authorization: Bearer $BO"
c "5 read the bronze customer back (was anything written?)"         "$H/customers/$GRACEID"
c "6 toggle as cy, a viewer without the registrar role"             -X "$METHOD" "$H$(R "$ADAID")" -H "authorization: Bearer $CY"
c "7 toggle with no token"                                           -X "$METHOD" "$H$(R "$ADAID")"
c "8 toggle a customer that does not exist"                          -X "$METHOD" "$H$(R nope)" -H "authorization: Bearer $BO"
c "9 list: which customers are active now"                           "$H/customers"
