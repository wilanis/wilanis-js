...
| `setup.sh <name> <port>` | One copy of `example/` under `.runs/<name>`: `node_modules` linked, `pinned` added to `Entry.shape.json` and `EntryRow.shape.json`, the http port set, `wilanis init` run, the skill and the Stop hook planted, `accept.sh` copied, and one git commit as the baseline. |
