...
function step0(ctx) {
  ...
  const out = check(ctx);
  assert(title, out === "ok: 200 documents", out, "check answers ok: 200 documents");
  return {
    number: 0,
    title,
    summary: out,
    ...
    answers: [{ pre: out }],
    why: "Every one of the 200 is a JSON document and none is code, so there is nothing an agent can write that the checker does not read whole. The two sentences above are the rules a human wrote; step 1 is where the first is caught, step 7 the second. Neither is repeated anywhere else in the tree.",
  };
}
