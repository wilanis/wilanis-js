...
| X406 | same | two queue triggers of the tree receive from the same connection (canonical) and queue: a delivery fires one, the first the tree lists, so each later one is refused on its own file at `settings/queue` (added by #640; this table was silent on it) | `one trigger per queue: remove <path>, or change its settings.queue to a queue no other trigger receives from` |
