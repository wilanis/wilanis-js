...
CUSTOMERS_API_KEY=        # secrets.customerKey, read by @connections/customers-api-production.connection.json
...
workloads:
  - profile: production
    ...
    env:
      - { name: CUSTOMERS_API_KEY, secretKey: CUSTOMERS_API_KEY }
...
secret:
  ...
  keys: [CUSTOMERS_API_KEY, CUSTOMERS_JWT_SECRET]
requires:
  - connection: "@connections/customers-api-production.connection.json"
    kind: "@http/http.connection-kind.json"
    endpoint: "https://customers.internal/api/v1"
...
docker: built customers:0.1.0
helm: installed customers (charts/wilanis-tree -f example/deploy/values.yaml -f charts/wilanis-tree/values-local.yaml)
ready: deployment/customers-production 1/1
...
(`test/fixtures/customers.manifest.json`, written by `wilanis manifest example` and checked in, so the package
...
generated values, in the `cluster` workflow, which also runs `scripts/cluster.sh up`, curls `GET /customers`
